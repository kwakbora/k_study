...
함수 외부에서 함수 내부로 값을 전달하는 방법이 딱 하나 있는데 바로 매개변수를 이용하는 것이다.<br />
매개변수라는 이름에서 알수 있듯이 일종의 외부 데이터를 함수 내부로 전달하는 매개체 역할을 하는 변수이다.
```javascript
def greetings(name);
	print("Hello", name)
...
리턴값은 매개변수와 반대의 개념이다. 매개변수는 함수 외부에서 함수 내부로 데이터를 전달하기 위해 사용하는 통로하면 리턴값은 함수 내부에서 함수 외부로 데이터를 보내기 위해 사용하는 통로이다
```javascript
def function(x):
  a = 3
  b = 5
